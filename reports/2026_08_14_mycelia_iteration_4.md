...
- Every count comes from `uv run hp query <name>` with `project=/Users/nob/repos/mycelia, since=NULL, as_of=2026-08-14, window_days=28` unless stated; non-default `--param` bindings and pinned `--as-of` dates are inline with each count. All queries exclude 4 sessions with no `project_dir`
