...
| `compaction/` | **As built,** the same `1de7cf38…` session the chained-message row named, which slice 1 never needed: two of its 18 boundaries with the `isCompactSummary` record each wrote | 2.1.198 | one `manual` and one `auto` compaction, so the trigger is a value rather than a constant |
...
| `legacy_title/` | **As built,** `0b34d1b8-ebd3-40a6-bd89-f1881e1de2ba.jsonl` — a 10-record session, where the 43-record `716eac06…` this plan named carries the same two title facts | 2.1.196 | `ai-title` and no `custom-title`. The name is a misnomer: `ai-title` is not legacy. Sessions write it through 2.1.221, the newest on this machine; `custom-title` is the operator's rename, and 13 of the 398 titled mycelia sessions hold both |
...
| `fork_copied/` | — | — | **As built: not built.** Its two jobs — a copied-history pair and a nested `spawnDepth: 2` run — are both in `fork_origin/`, and a third fork tree would have bought nothing |
| `fork_origin/` | **As built,** `5a88789c…/subagents/`: the auditor `agent-acbc29008a04b9702` and the fork `agent-a61a059e3610e6fb4` that copied it | 2.1.215 | a fork beside the transcript it copied: 12 shared records, then 4,904 output tokens of the fork's own work. The pair the plan first named (`a61a059e` copied onward into `ac7625b7`) carries only `attachment` records, so it cannot show a token rollup |
| `fork_byref/` | `07a769d7-828c-4edb-b3ce-af51e2712aa3/subagents/agent-afa3946951a08a798.jsonl` | 2.1.202 | opens with `fork-context-ref` carrying `parentSessionId`/`parentLastUuid`/`contextLength` |
...
| `resume_pair/` | `0a76f771-5f5b-447e-852a-664fc972ea7c.jsonl` (2.1.205) and `2352492b-1437-4427-ad51-70f35c75f663.jsonl` (2.1.202–205) | as noted | independent session files sharing an early conversation prefix verbatim, `message.id` and timestamps intact. **As built,** each excerpt keeps its own opening records as well as the shared block: without them both sessions start in the same millisecond, and `first_seen`'s tie-break would crown the resume rather than the session it copied |
...
- **As built, after slice 4:** a reply carrying a `fallback` block records the model Claude Code asked for first in `ApiCall.fallback_from`, and every other call leaves it `None`. The record names only the model that answered, so without the block a retry is invisible. *Evidence:* `server_tools/`, CC 2.1.206 — one of the corpus's 3 fallbacks.
...
- **As built, after slice 4:** an unregistered **content block** crashes too. The registry closed over record types only, so three block types went unread in silence. *Evidence:* invented fixture (labeled); assert the raise names the block type.
...
- A `schema_version` mismatch crashes on open with a message telling the operator to delete the DB and re-extract. *Evidence:* open a DB, rewrite `meta.schema_version`, reopen; assert the raise and that the message contains "re-extract". **As built,** rewriting the version cell alone left the tables current, so the test passed while the real path failed: opening ran the DDL first and altered the file before refusing. The leaf now opens a genuinely old-shaped store and asserts every table and column survives the refusal — the store is an archive, and the tool must not write to one it will not read.
...
- **As built, after slice 4: a `server_tool_use` block is a `tool_calls` row like any other, flagged `server_side`, and keeps its own clock.** Anthropic ran it, so its record is the request going out rather than a queue position: `duration_synthetic` is false, and the call ends when the `advisor_tool_result` block in the same message was written. *Evidence:* `server_tools/`, CC 2.1.201 — a message issuing two local calls and one server-side call, and a second holding an `advisor` call nothing ever answered. Bolded: while the block was unregistered it produced no row, no text and no crash, so 5 sessions that used the advisor read as sessions that had not.
...
- A nested subagent carries its `parent_agent_id` and a `spawn_depth` above 1. *Evidence:* `fork_copied/` (spawnDepth 2); assert both fields. **As built,** `spine/`'s own nested agent is depth 2, so the assertion rides its test and no fork fixture is pulled in.
- **A copied-history fork flags replays first-seen-wins, never symmetrically.** *Evidence:* `fork_origin/` — the 10 records native to `a61a059e` and copied into `ac7625b7`; assert every one of those rows is `replayed=False` under `a61a059e` and `replayed=True` under `ac7625b7`, and that no record is `replayed=True` in both. Bolded: symmetric flagging zero-counts work that really happened, in a session that exists today. **As built,** the pair is the auditor `acbc29008a04b9702` and the fork `a61a059e` that copied it — the same claim over records that carry tokens, so one fixture serves both this leaf and the rollup below.
- **Transcript ordering is `(spawn_depth, first-record timestamp, agentId)`, main first, a meta naming no depth last.** *Evidence:* `fork_origin/`, whose two transcripts share their first record and so tie on time; assert the auditor owns the copied rows and the fork replays them. Bolded: the ordering *is* the ownership rule — every count downstream follows it.
  - **As built,** after the first-record-timestamp rule this plan originally named was falsified. A copied-history fork's first record *is* its parent's, uuid and timestamp alike, so the tie is the norm: 46 of the machine's 51 overlapping transcript pairs tie on first timestamp, and an agentId tie-break flags 335 records of six real non-fork transcripts as replays. Depth separates them, a fork being spawned by the file it copies. Corpus check under the built rule: 1617 replayed records across 9 sessions, none in a non-fork transcript.
- `AgentRun.started_at` is the first non-replayed record, not the replayed history's first timestamp. *Evidence:* `fork_origin/`'s `ac7625b7`; assert `started_at` falls after the copied block's timestamps. **As built,** on `a61a059e`: its run starts at 22:05:03, 18 seconds after the auditor's transcript it copied began.
...
- The two fork variants are told apart by their leading record, not by their meta. *Evidence:* `fork_copied/` and `fork_byref/` — assert both metas carry `agentType: "fork"`/`isFork: true`, and that only the by-reference one gets a `fork_context_uuid`. **As built,** `fork_origin/` stands in for `fork_copied/`.
- A cross-transcript uuid overlap in which **neither** transcript is a fork crashes; an overlap involving a fork does not. *Evidence:* the crash from an invented pair (labeled — zero such pairs exist in 40 real overlap pairs); the non-crash from `fork_copied/`, whose parent is not a fork and which must parse clean. **As built,** the crash case is planted rather than invented: `fork_origin/`'s fork meta with `isFork` deleted, which is a real pair minus one key. All 51 real overlap pairs have a fork on one side.
...
- A `system/compact_boundary` record yields a `Compaction` with trigger, pre/post tokens, and duration from `compactMetadata`. *Evidence:* `chained_message/` (18 boundaries); whole-object assertion on one, plus a count equal to the fixture's `compact_boundary` records. **As built,** on `compaction/` and on both its boundaries, so `trigger` is asserted as `manual` on one and `auto` on the other. A `Compaction` also carries `source`: subagents compact far more often than main transcripts do, and the boundary belongs to the file that hit the limit.
...
- Title comes from the last `custom-title` record, and from `ai-title` on a legacy session. *Evidence:* `spine/` trimmed to two `custom-title` records (assert the later wins) and `legacy_title/` at 2.1.187. **As built,** "legacy" is wrong — see the fixture row. The precedence the code implements is by *authorship*, not by version: the operator's rename beats the one Claude Code generated.
- `agent_name` comes from the last `agent-name` record. *Evidence:* `spine/`.
- `pr-link` records become `PrLink` rows keyed by transcript line number, so a repeated `pr_number` in one session yields two rows. *Evidence:* `spine/` with two pr-link records sharing a number; assert both rows and their distinct `line_no`. **As built,** a `PrLink` carries no `source`: the 2,885 pr-link records on this machine sit in main transcripts only, none in the 2,464 subagent and journal files (scanned 2026-08-07), so the parser reads them from the main transcript alone.
...
- A `<synthetic>` model prices at zero and sets `synthetic=True`. *Evidence:* one of the 98 real `"model":"<synthetic>"` assistant records in the main-dir corpus, redacted into `spine/`. **As built,** the whole corpus holds 205 of them (the 98 counts main transcripts only); the redacted record joins `spine/`, whose whole-object test asserts the flag and the zero cost together.
...
- A model absent from the pricing table does not silently price at zero. *Evidence:* assert the behavior the implementer chooses — see *Obligations the seams can't reach*, where this contract gap is flagged; the leaf is not dischargeable until the design says which. **As built,** the design says `NULL` (manager decision), so the leaf discharges in two halves: `compute_cost` returns `None` for an unknown model, and the rollup reports `unpriced_api_calls` beside the sum so a total is never read as complete.

### integration (exporter)

- **`corpus_rollups` dedups turns, API calls, and tool calls alike, attributing each to its first-seen session.** *Evidence:* `resume_pair/` — export both sessions, whose shared prefix carries identical `message.id`s and timestamps; assert the corpus totals count the shared turns, API calls, and tool calls once each, attributed to whichever session the fixture's own timestamps make first (read them, don't assume — which of the pair is the ancestor is not established), while each session's own `session_rollups` still reports what its own file recorded. Bolded: revision 2 deduped API calls only, so copied tool calls and turns double-counted at corpus level — 156 tool_use ids span two or more main transcripts. **As built,** the view dedups agent runs and compactions too, on the same argument: a resume copies the whole file, `subagents/` included. The fixture pair shows it — the resume's own rollup reports 5 calls, 5 tool calls and a compaction, its corpus rollup the single call it added.
