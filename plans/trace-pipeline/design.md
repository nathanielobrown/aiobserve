...
- **Copied-history forks** (meta `agentType: "fork"` / `isFork: true`; both fork variants share this meta — the leading record distinguishes them): a uuid appearing in several transcripts of one session is native to the **first-seen transcript** (transcripts ordered by `(spawnDepth, first-record timestamp, agentId)`, main first, a meta naming no depth last) and `replayed=True` everywhere later — never symmetric flagging, which would zero-count work that originated in a fork and was then copied onward (25 of the 51 overlapping pairs are fork-to-fork, e.g. session `5a88789c`, where fork `a61a059e`'s records reach `ac7625b7` — 10 attachment records, so no tokens ride on this particular one). Depth has to lead the ordering: a copied-history fork opens on the record it copied, so 46 of the 51 pairs tie on first timestamp, and an agentId tie-break hands 335 records of six real transcripts' work to the fork that copied them. A fork is spawned by the transcript it copies, so it is always the deeper of the two. Replayed rows stay in the DB (the viewer shows the fork as recorded) but every rollup view excludes them, and `AgentRun.started_at` is the first non-replayed record. A cross-transcript uuid overlap where **neither** transcript is a fork crashes — every overlapping pair in the corpus involves a fork
...
A `user` record that is not `isMeta`, not `isSidechain` (condition dropped inside subagent transcripts, where every record is sidechain), not `isCompactSummary`, with either block content containing text/image and no `tool_result`, or non-empty string content classified by a closed tag registry. Census of every string that would otherwise pass — main-dir main transcripts: 968 plain prompts and 317+112 slash-command records, versus 2,157 `<task-notification>`, 279 `<local-command-stdout>`, and 11+11 `<bash-stdout>`/`<bash-input>` — an unfiltered rule counts ~3.6x too many turns and fills `Turn.prompt` with notification XML. Subagent transcripts (all 2,584, worktrees included) add exactly one further tag: 133 `<teammate-message>`. So:
...
- `meta(schema_version)` — checked on open; mismatch crashes with "delete the DB and re-extract". No migrations while the project is early. **As built,** the check runs on the untouched connection before any DDL: opening a store an older schema wrote refuses without adding a single current-shape table to it, so pointing the tool at a kept archive cannot alter it
...
- **Flag fork replays first-seen-wins, exclude from rollups** — keeps the archive faithful to what the file recorded while keeping every count single-counted, including work that originated in a fork and was copied onward. Rejected: dropping replayed rows (the viewer loses the fork's recorded context); ignoring them (double-counted tokens and cost); symmetric flagging (zero-counts records that only a fork holds, real in the corpus's 25 fork-to-fork overlaps).
- **Order transcripts by spawn depth before time** — a fork's first record *is* its parent's, so time ties on 46 of the 51 overlapping pairs, and the fork is by construction the deeper file. Rejected: first-record timestamp with an agentId tie-break (misattributes 335 records across six non-fork transcripts, which is also the corpus check the rule has to pass).
...
- **A model the pricing table lacks costs `NULL`, and never crashes** (manager decision) — `compute_cost` returns no cost for an unpriced model; the row keeps the model name and the cost columns stay `NULL`. The pricing table is ours, not Claude Code's schema, so a new model name is not a schema change to surface, and a model released mid-backfill must not kill the backfill. `NULL` is a queryable, documented absence: a query can find every unpriced call and the table can be filled in later. Rejected: crashing (fail-fast guards the transcript schema, not our own price list); pricing at zero (reports a free session, which is the prior importer's issue #3).
...
- **Re-extracting a pruned session from its archived `raw_records`** — a parser upgrade reaches only sessions whose files are still on disk, so a session Claude Code has pruned keeps whatever the parser of its day produced. The lines are all in the DB, so a re-parse from `raw_records` is possible; building it is deferred until an upgrade actually strands rows worth recovering.
