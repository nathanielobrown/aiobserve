...
Designed against the canonical store (schema 7, probed 2026-08-08, 575 sessions from the mycelia corpus): 294,489 spans corpus-wide under the mapping below — 571 contentful roots + 3,973 live turns + 130,559 live api calls + 155,845 live tool calls *no run named as its launch* + 2,487 agent runs + 1,054 copied-prefix-live compactions, measured 2026-08-08 by the slice-3 census against the store. The tool and run terms are **not** one suppressed call traded for one run, which is what a hand-written formula reaches for and what this line said until the census disagreed with it: 6 groups of runs share one spawning call (174 runs beyond one-per-call, the largest 93 from a single `Workflow` call in `8d930c77`), and 43 fork self-copies put a second suppressed row behind one run. 29,255 in the biggest session (`f1a1eb9a`), re-verified 2026-08-08 under the corrected mapper; 6 tool calls with `ended_at IS NULL`, 57,460 with `duration_synthetic`, 46 orphan agent runs, 1,229 live api calls with `turn_id IS NULL`, 217 live api calls under *replayed* turns (5 sessions, 17 fork sources), 204 `<synthetic>` api calls, 45 server-side tool calls, 4 sessions with no timestamps — exactly the 4 with `project_dir IS NULL`, all childless. Prior art and its failure record: `/Users/nob/repos/mac_settings/claude-otel/import_transcripts.py` (0.18.0) and its `investigations/pipeline-issues.md` — issues #1, #2, #6, #7 are the constraints here.
...
1. `StoreSource.sessions(project)` — reads `extract_state` joined to `sessions`, filtered to `project_dir` at or under `project`; returns `SessionSource(id, files=(), fingerprint=extract_state.fingerprint)`. Sessions with `project_dir IS NULL` are excluded by construction — all 4 in the corpus are childless bookkeeping shells — and `sessions()` crashes if a NULL-project session holds child rows, so the exclusion stays a bounded absence rather than a silent drop. The mapper's population is therefore what this filter can place: corpus-wide test sweeps parametrize over `exportable_transcripts()`, since the excluded sessions record no timestamps and shaping one is the schema-drift crash below
...
| ApiCall | `chat {model}`, CLIENT | its turn's span; turn **replayed** ⇒ the source run's span (217 calls in 17 fork sources sit under turns the fork replayed, whose first-seen copy hashes a different `source` — without this arm they dangle); `turn_id IS NULL` ⇒ the source run's span (fork continuation), root if main | tokens, cost, `stop_reason`, `effort`, `gen_ai.*` semconv as prior art; `<synthetic>` calls (204) ship with `hyphae.synthetic = true` |
| ToolCall with matched AgentRun | `invoke_agent {agent_type}` | the chat span | timed to the run's own `started_at..ended_at` (the launch-ack is ~0ms); the run's turns/calls nest under it; suppression is keyed by the **call's id**, so a fan-out's one call row goes once while every run it spawned emits its own span, and a fork's replayed copy of that row goes with it; NULL run times (model permits, 0 in corpus) ⇒ crash — a shape we need to see |
| ToolCall otherwise | `execute_tool {name}` | the chat span | `ended_at IS NULL` (6) ⇒ end = start + `hyphae.incomplete = true`; `duration_synthetic` (57K) and `server_side` (45) ship as attributes, never as invented times |
| AgentRun, orphan (46) | `invoke_agent {agent_type}` | root | `hyphae.orphan = true`; same id key as matched runs. Orphan means the trace holds no **live** copy of the spawning call, so a run whose call survives only as a fork's replayed copy is an orphan too. A run whose spawning call sits *inside its own transcript* is not: `fork_origin`'s recorded fork parents to `parent_agent_id`'s span (root when that is `main`) and carries no orphan flag, since hanging it off a call its own descendants recorded would cycle |
...
3. **Delivery hardening + named backends.** Multi-batch sessions, token bucket, gzip, retry/backoff, the honeycomb/logfire registry entries, `spans_sent` bookkeeping. Verified by receiver tests (429-then-success; batch counts; inter-batch pacing) and a corpus dry-run count checked against the mapping-true formula (contentful roots + live turns + live api calls + live tool calls no run named as its launch + all agent runs + copied-prefix-live compactions — 294,489 today; the compaction term is computed by the mapper's rule, not `live_compactions`, and the dry run also asserts every within-session duplicated compaction `id` keeps exactly one live copy — true 2/2 today, guarded because forks keep landing).
