...
- **A second `enrich()` with nothing changed submits nothing and writes nothing.** *Evidence:* the fake's call log is empty on the second pass and `enriched_at` is unchanged on every row. Bolded: this is what makes `enrich` safe to run beside `extract` on a schedule, and it is the cheapest possible regression net for the whole staleness scheme.
  - *As built:* runs over the forest store rather than `spine/`, and checks `enriched_at` at all three levels — see the fork self-copy leaf in slice 3.
...
- Each turn prompt is capped at 4K independently, not the sequence as a whole. *Evidence:* the extended `teammate/` fixture at an injected 200-char per-prompt cap; assert both prompts are present and each is truncated.
  - *As built:* the injected cap is 4 characters, not 200 — redaction leaves every recorded prompt at ten characters, so 200 cannot bite. The assertion reads four characters of each instruction: a cap counts its `[+n chars]` marker against its own limit, and a limit too small to hold the marker drops the marker rather than the text.
...
- **A run whose records name no parent agent is still ordered under the run whose transcript spawned it.** *Evidence:* planted (labeled) by nulling `parent_agent_id` on `spine/`'s leaf run, whose spawning call sits in its parent's transcript; assert the whole parent map is what it was with the column set. 112 of 2,459 recorded runs carry the shape, and no fixture does. Bolded: `parent_agent_id` alone strands every one of them at the root, sending it before the prompt that embeds it.
  - *As built:* a leaf the plan never carried, added 2026-08-07 after an audit mutated the union rule down to `parent_agent_id` and the whole suite stayed green. Discharged by `test_a_run_naming_no_parent_agent_hangs_off_the_transcript_that_spawned_it`. The ordering leaf above says ordering is over the union rule, but its evidence exercises only `parent_agent_id` edges.
- **A fork's replayed copy of its own spawning call carries no child description, so an unchanged store stays silent.** *Evidence:* the forest store, which holds `fork_origin/`'s self-copy; enrich twice and assert the second run sends nothing and rewrites no row at any level. 43 recorded runs hold such a copy. Bolded: a self-copy that carried a description would embed a run's description in its own prompt, so its hash never settles — perpetual re-enrichment, re-billed every scheduled run.
  - *As built:* also added 2026-08-07 after the audit; dropping the exclusion left the suite green. Slice 1's second-run leaf was the natural home and its store was `spine/`, which holds no self-copy, so that leaf now runs over the forest and discharges both.
...
- A session renders its title, branch, wall and active time, and token and cost totals, then its children in chronological order. *Evidence:* `spine/`; assert the whole rendered string. Note that branch comes from `sessions.git_branch` — `session_rollups` has no such column (verified on `data/traces_v6.duckdb`), so the render joins.
  - *As built:* the join holds. Two things the whole-string assert also pins: chronological order is not index order — `spine/`'s children render 3, 0, 1, 2 — and wall time is 30 days against 3m 39s active, because the session was resumed a month later. A duration helper renders the two largest units that carry it, so both numbers stay comparable at a glance.
- **A session's children are its main turns plus its rootless runs, each once.** *Evidence:* `teammate/`, which has exactly one of each — main turn `97d6f3d4…` and rootless run `aarchitect-…`; assert both descriptions appear, and separately assert on `spine/` that `ac461ef46b4bb8e32` (spawned by a main turn) does **not** appear directly, reaching the session through its turn instead. Bolded: audit finding B1 — the depth-1 reading dropped all 43 recorded teammate agents from every session summary and double-embedded 10 others.
  - *As built:* **the rule is not `tool_use_id IS NULL`** — see the design's slice-4 note. A session's direct children are the runs nothing else embeds, derived from the same union parent rule the rounds order by, which reaches all 55 rather than 46. The fixture corpus has no run in the recorded 9-run shape, so it is covered by a second leaf: `test_a_run_spawned_outside_every_turn_is_still_a_session_child` plants `turn_id = NULL` on one recorded spawning call and asserts the run becomes a session child. Labeled planted in the test, as `.claude/rules/testing.md` requires.
- A session with more children than the budget allows elides the middle. *Evidence:* `spine/`'s four children at an injected budget of two; assert head, tail, and marker. Real sessions average 2.5 children and reach 92.
  - *As built:* the injected budget is 300 characters, not two children — the render's unit of elision is a line and its budget is a character count, as at the other two levels. The average is 3.1, not 2.5: the wider children rule adds the 9. Max 92 stands.
...
- `enriched_agent_runs` and `enriched_sessions` LEFT-join like `enriched_turns`, so coverage reads honestly. *Evidence:* enrich one run of two; assert both rows return, one with NULL enrichment.
  - *As built:* `agent_runs` carries a `description` and a `model` of its own, which would collide. The view excludes both and re-exposes them as `task_description` and `agent_model`, so `description` means the enrichment's in all three views; the leaf asserts those names exist. The run's key column stays `id`, the base table's name.
- A session with no main turns and no runs is never enriched. *Evidence:* `compaction/` and `dup_uuid/` in the DB; assert `session_enrichments` holds no row for either, and that the enricher reports them as skipped rather than failed. 102 of 575 real sessions are in this state, so coverage is 473.
  - *As built:* there is nothing to report as skipped. Such a session is not an item at all — `session_items()` filters it out — so no round sends it, no row is written, and no failure can name it. The leaf asserts the store's empty sessions are exactly `compaction/` and `dup_uuid/` and that neither is among the items, which is the stronger claim: a skip that leaves a trace would be a mechanism that could get one wrong.
...
- **`--dry-run` counts hash-stale items plus all their ancestors, and says the number is an upper bound.** *Evidence:* enrich `spine/` fully, make only leaf run `af6473ae437c9608d` hash-stale, and assert `--dry-run` reports four items (the leaf, its parent run, the spawning main turn, the session) while the subsequent real run may write fewer. Bolded: a dry run that reports one item quotes a price for a cascade it cannot see.
  - *As built:* four items exactly, and both words are printed — "at most N item(s)", "at most $X batched". Extending `item_parents()` to all three levels is what reaches the session, and it discharges three deferred halves at once: this count, the cascade-to-session leaf below, and slice 2's blocked-parents leaf, which now asserts a failed leaf leaves the session unwritten too.
  - *Watch for in the audit:* the cascade only climbs a level when the child's **description** changes, not when its input does. `test_a_childs_new_description_makes_its_ancestors_stale` first read green three levels deep with the session untouched, because the turn was re-described in identical words. The test now scripts new text at every level it claims to reach. A future leaf that asserts a cascade without changing the description will pass while proving nothing.
- The cost estimate is arithmetic over rendered character counts and the code-resident price table, with no network and no live lookup. *Evidence:* a fixture DB whose rendered sizes the test computes itself; assert the printed estimate equals the hand-computed product of those sizes, the chars-per-token constant, and the table's rates, and assert the run makes no API call.
  - *As built:* split across `tests/enrich/test_cost.py` (the arithmetic, over stated lengths) and an end-to-end leaf asserting the printed dollars equal what `estimate` derives from the same plan. "No API call" needs no assertion of its own — the autouse `refuse_network` fixture raises on any HTTP, so an implementation that looked a price up would fail rather than pass. Instruction cost is derived from the real `instructions(level)` text, replacing the design's ~700-token constant, and `CHARS_PER_TOKEN` takes the low end of the measured 3.3–4 so the quote reads high.
- `cli.main("enrich", …)` drives the same path as `enrich()` and produces the same DB. *Evidence:* invoke in-process (no subprocess, no timeout to bound); compare every enrichment table against a direct `enrich()` run.
  - *As built:* compares all three enrichment tables, `enriched_at` aside — the one column a second run cannot reproduce, as in slice 2's client-equality leaf.
- `--no-batch` selects `SyncClient` and `--dry-run` works under both. *Evidence:* assert the client class the CLI constructed, through the same protocol seam.
  - *As built:* the client half was already discharged in slice 2 by `test_the_batch_flag_picks_the_client`. The new leaf covers the dry-run half: both invocations print byte-identical reports, and neither builds a client — `build_client` is left unpatched, so constructing one would reach for a key a dry run has no business spending.
