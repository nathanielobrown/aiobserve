...
- A bare invocation runs the manifest's production defaults, and an explicit `--param` overrides one without disturbing the rest. *Evidence:* run `select_sessions` with no `--param` and assert the citation reports the production quotas; run it again overriding one quota and assert only that binding changed.
...
The digest queries through the CLI, against the fixture store; the truncation leaf uses a copy with a planted sentinel.
