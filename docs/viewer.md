...
Rows show only the head of long text: 100 characters for the title and project path, four skill names and four agent types followed by the number omitted, and three kinds of work. The session header shows five skill names of up to 60 characters along with its other fields; it too stays bounded. A name or PR URL longer than that ends in an ellipsis like every other cut value, and a PR URL the page had to cut is printed rather than linked: half a URL points somewhere else.
...
- an **api call**: the head of the text it answered with, else the model that answered — a call that answered with tool calls and no text has nothing to quote
...
Full-value requests are the declared exception. Each returns one transcript line, prompt, task brief, text block, thinking block, or tool value, so its size depends on the largest matching value rather than a page of them. The tail row's fetch is the other: a reader who clicks `+N more` is asking for the rest of that level, so it serves the level less the window at a tree row apiece — 1.8 MB for the widest level in the canonical store, 1,573 api calls under one turn. Offloads remain chunked. JSON is re-indented only while doing so remains cheap; deeply nested data stays as stored because indentation work grows quadratically with nesting.
