...
One page keeps a measured claim rather than an arithmetic one: a session's timeline renders every turn the session has, so its size is bounded by the corpus rather than by construction. The largest recorded session serves 115 KB from 79 turns. `tests/view/test_bounds.py` holds the ceiling and everything that has to fit under it.
