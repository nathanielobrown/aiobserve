...
A run is named for the agent that ran it: `Explore — found the indent bug`. The agent type leads, and after it what an [enrichment pass](enrichment.md) said the run did, else the task brief it was given, else nothing. The lead is constant wherever a run is named — tree row, crumb, pane header, tab title — because which agent ran is what tells six runs of one turn apart. The exception is the unattached bucket's children log, which heads a column with the agent type: there the wide column holds what the run did and nothing else.
