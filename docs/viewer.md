...
A tool call's input is a fat column no page may load whole, so the store does the reading: one macro lifts the fields the naming rules read, each cut to the width its caller asked for, down to the head of the input as stored — the last field because it is the last resort (`src/hyphae/analyze/macros.py`). The naming itself is Python's, in `src/hyphae/view/formatters.py` — SQL cannot dispatch on a tool's name without a `CASE` arm per tool, and a tool absent there is not a gap but the fallback working. Every other kind is composed in `src/hyphae/view/builders.py` over what the query that read the node returned, an api call's tool calls reaching it as their names in order and the fields of the first. So the queries fetch the parts, the composition owns the sentence and its widths, each kind has one derivation, and the four widths above are the only cuts of it.
