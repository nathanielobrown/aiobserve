...
What a viewer URL may ask for, and what the page that comes back is allowed to weigh. [The viewer guide](viewer.md) says what each page shows; this one says what bounds it. `src/aiobserve/view/bounds.py` defines every number here.
...
The presets are the [control above the tree](viewer.md#the-tree-opens-one-path-and-nothing-else), and typing one into the URL does the same thing. Every preset leaves every visible node with a visible parent, and a level whose preset would hide the path you are standing on renders in full instead.
