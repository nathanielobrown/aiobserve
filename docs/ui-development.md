...
Three environment variables move where the tier points: the base URL, the server command, and the readiness URL (`tests/e2e/playwright.config.ts`). Unset, it is the gallery above and nothing has changed. Set, the same unedited specs drive another server, which is how the Rust prototype ran this tier against its own binary (`plans/rust-prototype/design.md`).
