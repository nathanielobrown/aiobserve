...
from hyphae.view.store import Fragment, Row, Value, open_store, page_rows
...
def routes(viewer: Viewer) -> list[BaseRoute]:
    ...
    def prose(value: Value, keyed: Mapping[str, ParamValue], column: str, detail: str) -> Response:
        ...
        an answer reads it.
        """
        row, citation = fetched(value, keyed, column)
    ...
    def enrichment_line(value: Value, keyed: Mapping[str, ParamValue], field: str) -> Response:
        ...
        row, citation = fetched(value, keyed, field)
    ...
    def record_value(session_id: str, source: str, line_no: int) -> Response:
        ...
        row, citation = fetched(Value.RECORD, keyed, "raw")
    ...
    def tool_input(session_id: str, source: str, tool_call_id: str) -> Response:
        """What one tool call was passed, whole."""
        keyed = {"session_id": session_id, "source": source, "tool_call_id": tool_call_id}
        return code(Value.TOOL_INPUT, keyed, "value", "input")

    @router.get("/fragment/result/session/{session_id}/thread/{source}/tool/{tool_call_id}")
    def tool_result(session_id: str, source: str, tool_call_id: str) -> Response:
        ...
        return code(Value.TOOL_RESULT, keyed, "value", "result")

    @router.get("/fragment/command/session/{session_id}/thread/{source}/tool/{tool_call_id}")
    def tool_command(session_id: str, source: str, tool_call_id: str) -> Response:
        """What one `Bash` call ran, whole — read as the shell reads it."""
        keyed = {"session_id": session_id, "source": source, "tool_call_id": tool_call_id}
        return code(Value.TOOL_COMMAND, keyed, "value", "command", highlight.Syntax.BASH)

    @router.get("/fragment/prompt/session/{session_id}/thread/{source}/turn/{turn_id}")
    def turn_prompt(session_id: str, source: str, turn_id: str) -> Response:
        """What one turn was asked, whole."""
        keyed = {"session_id": session_id, "source": source, "turn_id": turn_id}
        return prose(Value.TURN_PROMPT, keyed, "value", "prompt")

    @router.get("/fragment/args/session/{session_id}/thread/{source}/turn/{turn_id}")
    def turn_command_args(session_id: str, source: str, turn_id: str) -> Response:
        """What followed the slash command one turn ran, whole."""
        keyed = {"session_id": session_id, "source": source, "turn_id": turn_id}
        return prose(Value.TURN_COMMAND_ARGS, keyed, "value", "command_args")

    @router.get("/fragment/brief/session/{session_id}/run/{run_id}")
    def run_brief(session_id: str, run_id: str) -> Response:
        """The whole brief one agent run was given."""
        keyed = {"session_id": session_id, "run_id": run_id}
        return prose(Value.RUN_BRIEF, keyed, "value", "brief")

    @router.get("/fragment/prompt/session/{session_id}/run/{run_id}")
    def run_prompt(session_id: str, run_id: str) -> Response:
        """The whole of what one agent run was asked, off the call that spawned it."""
        keyed = {"session_id": session_id, "run_id": run_id}
        return prose(Value.RUN_PROMPT, keyed, "value", "prompt")

    @router.get("/fragment/result/session/{session_id}/run/{run_id}")
    def run_result(session_id: str, run_id: str) -> Response:
        """The whole of what one agent run sent back to the agent that spawned it."""
        keyed = {"session_id": session_id, "run_id": run_id}
        return prose(Value.RUN_RESULT, keyed, "value", "result")
