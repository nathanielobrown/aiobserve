...
The estimate counts prompt caching at zero and assumes every request pays for its own
instructions, so it reads high on that axis. It reads low on another — a run may cascade
further than the plan can see. Neither is a bound; quote it as an estimate.
"""
...
# The Message Batches API bills at half the list rate, which is why production runs batched.
BATCH_DISCOUNT = 0.5
...
# One answer is four short fields, and the schema caps the description. Measured from the
# live check in `tests/enrich/test_batches.py`, rounded up.
OUTPUT_TOKENS = 200
...
@dataclass(frozen=True)
class Estimate:
    """What a pass would cost, both ways it can be sent."""
...
def estimate(prompts: Sequence[Prompt], model: str) -> Estimate:
    ...
    unbatched = (input_tokens * rates.input_usd + output_tokens * rates.output_usd) / 1_000_000
    return Estimate(
        items=len(prompts),
        input_tokens=input_tokens,
        output_tokens=output_tokens,
        batched_usd=unbatched * BATCH_DISCOUNT,
        unbatched_usd=unbatched,
    )
