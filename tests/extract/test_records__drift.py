...
from aiobserve.extract import claude_code, records
...
def modelled() -> set[str]:
    ...
    return {
        *(model.RECORD_TYPE.value for model in records.RECORD_MODELS),
        *(model.SUBTYPE.value for model in records.RECORD_MODELS if model.SUBTYPE is not None),
        *(block.BLOCK.value for block in records.BLOCK_MODELS),
    }
...
def test_every_registered_shape_has_a_model_or_a_stated_reason(member: str) -> None:
    ...
    assert member in modelled() or member in records.UNMODELLED, (
...
def test_no_reason_is_left_for_a_shape_that_no_longer_exists() -> None:
    ...
    for kind, reason in records.UNMODELLED.items():
...
def test_every_documented_field_is_one_the_parser_reads() -> None:
    ...
    for field, path in documented_fields().items():
        ...
        assert read or field in records.OBSERVED_UNREAD, (
...
def test_nothing_is_excused_as_unread_that_the_parser_reads() -> None:
    ...
    for field, reason in records.OBSERVED_UNREAD.items():
