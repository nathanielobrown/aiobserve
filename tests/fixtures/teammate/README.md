...
- `subagents/agent-aarchitect-5144001ac50718bc.jsonl`, lines 1–6 of 93 — the teammate's own
  transcript, opening on a `<teammate-message>` from its team lead. That tag is the only turn opener
  that carries attributes, and no main transcript of the corpus holds one. The agent id is also the
  answer to whether a stem after `agent-` is always hex: this one is not
