...
- Record what you relied on: when a parser depends on a field's shape, declare that field on its record model in `src/aiobserve/extract/records/`, with the recording that shows it — `docs/schema.md` prints what the models carry
...
The record models in `src/aiobserve/extract/records/` are the exception: they exist to say what Claude Code's fields mean, so every field there carries a `description` and the `Cited` recording behind it, and one that carries neither fails the schema generator.
