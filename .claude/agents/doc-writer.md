...
- Documenting a telemetry field means declaring it on its record model in `src/hyphae/extract/records/`, naming the recorded session that shows it and the Claude Code version that produced it; `docs/schema.md` prints what the models carry. Never describe a schema from memory
